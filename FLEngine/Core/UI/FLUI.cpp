...
namespace FLUI
{
    ...
    void FLUI::applyStyle()
    {
        ...
        colors[ImGuiCol_DockingPreview]         = AdjustColor(preferedColor, 0.75f, 0.40f);
        colors[ImGuiCol_DockingEmptyBg]         = ImVec4(0.50f, 0.50f, 0.50f, 0.50f);
    ...
    void FLUI::preferedColorPushDrawCommands()
    {
        ...
        if (ImGui::Begin("Command Buffer Recording", NULL, ImGuiWindowFlags_AlwaysAutoResize)) 
        {
            ...
            if (ImGui::Checkbox("Fullscreen", &bFullscreenMode))
            {
                // Toggle fullscreen mode when the checkbox state changes
                window->toggleFullscreen(bFullscreenMode);
            }
